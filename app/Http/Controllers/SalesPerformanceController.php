...
class SalesPerformanceController extends Controller
{
    public function getSalesPerformance()
    {
        try {
            ...
            // IMPORTANT: Cast sales_id (bigint) to match user_id (integer)
            ...
            // Handle kalau tidak ada data sales
            ...
            // Generate dummy data untuk Deal & Fails
            // Nanti kalau udah ada table/logic nya, ganti dengan query real
            $dummyDeals = [];
            $dummyFails = [];
            
            foreach ($salesPerformance as $sales) {
                // Dummy: random 50-90% dari company visited bisa jadi deal
                $dealPercentage = rand(50, 90) / 100;
                $deals = (int) floor($sales->company_visited * $dealPercentage);
                
                // Sisanya jadi fails
                $fails = $sales->company_visited - $deals;
            ...
            $totalDeal = array_sum($dummyDeals);
            $totalFails = array_sum($dummyFails);
            ...
            return response()->json([
                ...
                'note' => 'Deal and Fails data are currently dummy/simulated'
            ]);
    ...
    public function getSalesDetail($userId)
    {
        try {
            ...
            $sales = DB::table('users')
                ...
                ->where('users.role_id', 12)  // Pastikan role Sales
            ...
            // Get Company yang sudah dikunjungi sales ini
            // CAST sales_id untuk match dengan user_id (integer)
            ...
            // Generate dummy deal/fails per company
            $companyWithStatus = $companyVisited->map(function($company) {
                // Random: 50-80% chance untuk Deal
                $rand = rand(1, 100);
                if ($rand <= 70) {
                    $status = 'Deal';
                    $statusColor = 'green';
                ...
                return [
                    ...
                    'dummy_status' => $status,
                    'dummy_status_color' => $statusColor
                ];
            });

            $dealCount = $companyWithStatus->where('dummy_status', 'Deal')->count();
            $failsCount = $companyWithStatus->where('dummy_status', 'Fails')->count();
            ...
            return response()->json([
                ...
                'note' => 'Deal/Fails status are dummy data for now'
            ]);
